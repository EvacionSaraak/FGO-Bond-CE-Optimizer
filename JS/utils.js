...
function translateJapaneseCEDetail(detail){
  ...
  if(text.includes("クエストクリア時")&&text.includes("絆")){
    if(supportMatches.length&&bondMatches.length){const own=Math.min(...bondMatches),support=Math.max(...supportMatches);return`Increases Bond gained after clearing quests by ${own}% (${support}% when used as support). Applies to ${conditionText}.`;}
    if(bondMatches.length)return`Increases Bond gained after clearing quests by ${Math.max(...bondMatches)}%. Applies to ${conditionText}.`;
  }
  const replacements=[
    ["自身に","Apply to self: "],["自身の","Own "],["クエストクリア時に得られる","When clearing quests, gained "],["絆を","Bond "],["増やす","increases"],["アップ","up"],["付与","grants"],["状態","state"],["バトルを開始する","at battle start"],["NPを","NP "],["NP獲得量を","NP gain "],["宝具威力を","NP damage "],["クリティカル威力を","critical damage "],["カード性能を","card performance "],["スターを","stars "],["個獲得"," gained"],["特攻","special attack"],["弱体無効","debuff immunity"],["登場時に","when entering the field, "],["1回","1 time"],["チャージした","charged"],["状態で"," with "],["＆","; "],["＋","; "],["〔","["],["〕","]"],["かつ"," + "],["または"," / "],["％","%"]
  ];
...
function createClassIcon(className){
  ...
  const canvas=document.createElement("canvas");canvas.width=CLASS_ICON_SIZE;canvas.height=CLASS_ICON_SIZE;
  ...
  const dataUrl=canvas.toDataURL("image/png");classIconCache.set(normalized,dataUrl);return dataUrl;
}
