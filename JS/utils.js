...
function extractBondPercents(detail, ceName = "") { const original = String(detail || ""); if (!isBondBoostCE(original)) return { basePercent: 0, mlbPercent: 0, ownBasePercent: 0, ownMlbPercent: 0, isSupportConditional: false }; const jpBondMatches = [...original.matchAll(/絆[^0-9０-９]*(\d+|[０-９]+)\s*[%％]/g)], enBondMatches = [...original.matchAll(/(?:bond|friendship)[^0-9]*(\d+)\s*[%％]/gi)], supportMatches = [...original.matchAll(/(?:サポート|support)[^0-9０-９]*(\d+|[０-９]+)\s*[%％]/gi)]; const values = [...jpBondMatches, ...enBondMatches].map((m) => toAsciiNumber(m[1])).filter(Boolean), supportValues = supportMatches.map((m) => toAsciiNumber(m[1])).filter(Boolean); const basePercent = supportValues.length ? Math.max(...supportValues) : values.length ? Math.max(...values) : 0, ownBasePercent = values.length ? Math.min(...values) : basePercent, isSupportConditional = (supportValues.length > 0 && ownBasePercent !== basePercent) || normalizeText(ceName) === "chaldea teatime"; return { basePercent, mlbPercent: basePercent * 5, ownBasePercent, ownMlbPercent: ownBasePercent * 5, isSupportConditional }; }
...
function doesCEAffectServant(ce, servant, ceSlotIndex, servantSlotIndex, ignoreExceptSelf = false) { if (!ce || !servant) return false; const jpConditionGroups = getJapaneseBondConditionGroups(ce.detail || ""); if (jpConditionGroups.length) return jpConditionGroups.some((group) => group.every((condition) => servantMatchesCECondition(servant, condition))); if (isGenericJapaneseBondCE(ce.detail || "")) return true; const description = ce.normalizedDetail || normalizeText(ce.detail || ""), exceptSelf = isExceptSelfCE(description); if (!ignoreExceptSelf && exceptSelf && ceSlotIndex === servantSlotIndex) return false; if (matchesPartyWideBondRule(description)) return true; if (description.includes(servant.normalizedName)) return true; if (description.includes(`${servant.className} class`) || description.includes(`${servant.className}-class`) || description.includes(`${servant.className} allies`) || description.includes(`${servant.className} servants`)) return true; if (servant.gender !== "unknown" && (description.includes(`${servant.gender} allies`) || description.includes(`${servant.gender} servants`) || description.includes(`${servant.gender} party`))) return true; if (servant.attribute !== "unknown" && (description.includes(`${servant.attribute} allies`) || description.includes(`${servant.attribute} servants`) || description.includes(`${servant.attribute} attribute`))) return true; return servant.traits.some((trait) => trait && description.includes(trait)); }
function getServantBondBonus(servantSlotIndex) { const servant = state.selectedServants[servantSlotIndex]; if (!servant) return 0; const partyBond15Bonus = state.selectedServants.reduce((sum, selectedServant, selectedSlotIndex) => { if (!selectedServant || selectedSlotIndex === servantSlotIndex || !state.selectedServantBond15[selectedSlotIndex]) return sum; return sum + 25; }, 0); const ceBonus = state.selectedCEs.reduce((sum, ce, ceSlotIndex) => { if (!ce || !doesCEAffectServant(ce, servant, ceSlotIndex, servantSlotIndex)) return sum; return sum + getCEBondPercent(ce, ceSlotIndex); }, 0); return ceBonus + partyBond15Bonus; }
