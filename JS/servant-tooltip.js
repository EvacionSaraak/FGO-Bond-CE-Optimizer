function initServantTooltip(){
  ...
  const moveTooltip=(event)=>{if(!tooltip.classList.contains("visible"))return;const margin=16,rect=tooltip.getBoundingClientRect();let left=event.clientX+margin,top=event.clientY+margin;if(left+rect.width>window.innerWidth-margin)left=event.clientX-rect.width-margin;if(top+rect.height>window.innerHeight-margin)top=event.clientY-rect.height-margin;tooltip.style.left=`${Math.max(margin,left)}px`;tooltip.style.top=`${Math.max(margin,top)}px`;};
  document.addEventListener("mouseover",(event)=>{const trigger=event.target.closest("[data-servant-id],[data-servant-tooltip-id]");if(!trigger)return;const servantId=Number(trigger.dataset.servantId||trigger.dataset.servantTooltipId),servant=state.servants.find((entry)=>entry.id===servantId)||state.selectedServants.find((entry)=>entry?.id===servantId);if(!servant)return;renderTooltip(servant);tooltip.classList.add("visible");moveTooltip(event);});
...
function initCETooltip(){
  ...
  const enrichCE=(ce)=>{if(!ce)return null;if(Array.isArray(ce.affectedServants))return ce;const affectedServants=state.selectedServants.map((servant,slotIndex)=>({servant,slotIndex,bonus:getHypotheticalCEBonusForServant(ce,servant,slotIndex)})).filter((entry)=>entry.servant&&entry.bonus>0),totalBonus=affectedServants.reduce((sum,entry)=>sum+entry.bonus,0);return{...ce,affectedServants,totalBonus};};
  const renderTooltip=(rawCE)=>{
    const ce=enrichCE(rawCE),tag=getCEEffectTag(ce),total=formatPercent(Number(ce.totalBonus)||0),affected=ce.affectedServants||[],effective=formatPercent(getOptimizedCEBondPercent(ce));
    const affectedHtml=affected.length?affected.map(({servant,slotIndex,bonus})=>`<div class="ce-tooltip-affected-row"><span>Slot ${slotIndex+1}</span><strong>${escapeHtml(servant.name)}</strong><b>+${formatPercent(bonus)}%</b></div>`).join(""):`<div class="ce-tooltip-muted">Affects no selected servants.</div>`;
    tooltip.innerHTML=`
      ...
      <div class="ce-tooltip-section"><div class="servant-tooltip-label">Detail</div><div class="ce-tooltip-detail">${escapeHtml(ce.detail||"No detail text.")}</div></div>
    `;
    bindImageFallbacks(tooltip);
  };
  const moveTooltip=(event)=>{if(!tooltip.classList.contains("visible"))return;const margin=16,rect=tooltip.getBoundingClientRect();let left=event.clientX+margin,top=event.clientY+margin;if(left+rect.width>window.innerWidth-margin)left=event.clientX-rect.width-margin;if(top+rect.height>window.innerHeight-margin)top=event.clientY-rect.height-margin;tooltip.style.left=`${Math.max(margin,left)}px`;tooltip.style.top=`${Math.max(margin,top)}px`;};
  document.addEventListener("mouseover",(event)=>{const trigger=event.target.closest("[data-ce-tooltip-id]");if(!trigger)return;const ce=findCE(Number(trigger.dataset.ceTooltipId));if(!ce)return;renderTooltip(ce);tooltip.classList.add("visible");moveTooltip(event);});
  document.addEventListener("mousemove",moveTooltip);
