...
function cacheDom(){
  ...
  dom.clearServantOptimizationButton=document.getElementById("clear-servant-optimization-btn");
  ...
  dom.teatimeOwnModeInput=document.getElementById("teatime-own-mode-input");
}
...
function bindEvents(){
  ...
  dom.clearServantOptimizationButton?.addEventListener("click",handleClearServantOptimization);
  ...
  dom.teatimeOwnModeInput?.addEventListener("change",(event)=>{state.chaldeaTeatimeAsOwn=Boolean(event.currentTarget.checked);if(Array.isArray(state.recommendations)&&state.recommendations.length)handleOptimizeCEs();renderAll();});
}
