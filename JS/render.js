...
function bindSlotEvents(container) {
  ...
  container.querySelectorAll(".remove-entry").forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      if (type === "servant") { state.selectedServants[index] = null; state.selectedServantBond15[index] = false; } else { state.selectedCEs[index] = null; state.selectedCEOwned[index] = false; }
      renderAll();
    });
  });
}
