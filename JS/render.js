...
function renderCostHeaders(){const servantCost=getSelectedServantCost(),ceCost=getSelectedCECost(),totalCost=servantCost+ceCost,maxCost=getMaxTotalCost();if(dom.servantLineupTitle)dom.servantLineupTitle.textContent=`Servants (Cost: ${servantCost})`;if(dom.ceLineupTitle)dom.ceLineupTitle.textContent=Number.isFinite(maxCost)?`Craft Essences (Cost: ${ceCost} / Total: ${totalCost} of ${maxCost})`:`Craft Essences (Cost: ${ceCost})`;}
function renderAll(){renderCostHeaders();renderServantSlots();renderCESlots();renderServantSidebar();renderCESidebar();renderRecommendations();bindImageFallbacks(document);}

function renderServantSlots(){
  dom.servantSlots.innerHTML=state.selectedServants.map((servant,index)=>{const totalBonus=getServantBondBonus(index),activeClass=state.activeServantSlot===index?"active-slot":"";return `<div class="selection-slot ${activeClass}" data-slot-type="servant" data-slot-index="${index}">${servant?`<button type="button" class="btn-close remove-entry" data-remove-type="servant" data-remove-index="${index}" aria-label="Remove servant"></button>`:""}${servant?servantSlotMarkup(servant,index,totalBonus):emptySlotMarkup("Servant",index)}</div>`;}).join("");
...
function bindSlotEvents(container){
  container.querySelectorAll("[data-slot-type]").forEach((button)=>{button.addEventListener("click",(event)=>{const type=event.currentTarget.dataset.slotType,index=Number(event.currentTarget.dataset.slotIndex);if(type==="servant"){state.activeServantSlot=index;state.activeCESlot=null;}else{state.activeCESlot=index;state.activeServantSlot=null;}renderAll();});});
  container.querySelectorAll(".remove-entry").forEach((button)=>{button.addEventListener("click",(event)=>{event.stopPropagation();const type=event.currentTarget.dataset.removeType,index=Number(event.currentTarget.dataset.removeIndex);if(type==="servant"){state.selectedServants[index]=null;state.selectedServantBond15[index]=false;state.selectedServantBondHidden[index]=false;}else{state.selectedCEs[index]=null;state.selectedCEOwned[index]=false;}renderAll();});});
}
...
function renderCESidebar(){
  ...
  const isLoading=state.ceSidebarLoading;dom.ceSidebar.classList.toggle("sidebar-loading",isLoading);dom.ceSearch.disabled=isLoading;if(dom.cePageSize)dom.cePageSize.disabled=isLoading;dom.ceSearch.setAttribute("aria-busy",String(isLoading));dom.ceResults.classList.toggle("sidebar-loading-results",isLoading);
...
function renderRecommendations(){
  const recommendations=Array.isArray(state.recommendations)?state.recommendations:[];state.recommendations=recommendations;
