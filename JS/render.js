...
function bindSlotEvents(container) {
  container.querySelectorAll("[data-slot-type]").forEach((button) => { button.addEventListener("click", (event) => { const type = event.currentTarget.dataset.slotType, index = Number(event.currentTarget.dataset.slotIndex); if (type === "servant") state.activeServantSlot = index; else state.activeCESlot = index; renderAll(); }); });
  container.querySelectorAll(".remove-entry").forEach((button) => { button.addEventListener("click", (event) => { event.stopPropagation(); const type = event.currentTarget.dataset.removeType, index = Number(event.currentTarget.dataset.removeIndex); if (type === "servant") { state.selectedServants[index] = null; state.selectedServantBond15[index] = false; } else { state.selectedCEs[index] = null; state.selectedCEOwned[index] = false; } renderAll(); }); });
}

function renderServantSidebar() {
  dom.servantSidebar.classList.remove("d-none"); const isLoading = state.servantSidebarLoading; dom.servantSidebar.classList.toggle("sidebar-loading", isLoading); dom.servantSearch.disabled = isLoading; dom.servantPageSize.disabled = isLoading; dom.servantSearch.setAttribute("aria-busy", String(isLoading)); dom.servantResults.classList.toggle("sidebar-loading-results", isLoading);
  const slotIndex = state.activeServantSlot; dom.servantSlotLabel.textContent = slotIndex !== null ? `Slot ${slotIndex + 1}` : "Any Slot";
  ...
  state.servantSidebarPage = currentPage; const pageStart = totalServants ? (currentPage - 1) * pageSize : 0, pagedServants = visibleServants.slice(pageStart, pageStart + pageSize);
  dom.servantPageSize.value = String(pageSize); dom.servantPageLabel.textContent = `Page ${currentPage} of ${totalPages}`; dom.servantPagePrev.disabled = isLoading || currentPage <= 1; dom.servantPageNext.disabled = isLoading || currentPage >= totalPages;
  ...
  dom.servantResults.querySelectorAll("[data-add-servant]").forEach((button) => { button.addEventListener("click", () => { const servantId = Number(button.dataset.addServant), servant = state.servants.find((entry) => entry.id === servantId); if (!servant) return; const targetIndex = getTargetServantSlotIndex(); if (!canAddServantToSelection(servantId, targetIndex)) return; const previousServantId = state.selectedServants[targetIndex]?.id; state.selectedServants[targetIndex] = servant; if (previousServantId !== servantId) state.selectedServantBond15[targetIndex] = false; state.servantOptimizationEnabled = false; renderAll(); }); });
}

function renderCESidebar() {
  dom.ceSidebar.classList.remove("d-none"); const isLoading = state.ceSidebarLoading; dom.ceSidebar.classList.toggle("sidebar-loading", isLoading); dom.ceSearch.disabled = isLoading; dom.cePageSize && (dom.cePageSize.disabled = isLoading); dom.ceSearch.setAttribute("aria-busy", String(isLoading)); dom.ceResults.classList.toggle("sidebar-loading-results", isLoading);
  const slotIndex = state.activeCESlot; dom.ceSlotLabel.textContent = slotIndex !== null ? `CE Slot ${slotIndex + 1}` : "Any Slot";
  ...
  state.ceSidebarPage = currentPage; const pageStart = totalCEs ? (currentPage - 1) * pageSize : 0, pagedCEs = visibleCEs.slice(pageStart, pageStart + pageSize);
  if (dom.cePageSize) dom.cePageSize.value = String(pageSize); if (dom.cePageLabel) dom.cePageLabel.textContent = `Page ${currentPage} of ${totalPages}`; if (dom.cePagePrev) dom.cePagePrev.disabled = currentPage <= 1; if (dom.cePageNext) dom.cePageNext.disabled = currentPage >= totalPages;
...
function renderRecommendations() {
  ...
  dom.recommendationArea.innerHTML = recommendations.map((ce) => recommendationMarkup(ce)).join("");
