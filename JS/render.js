...
function renderServantSlots(){
  ...
  dom.servantSlots.querySelectorAll("[data-servant-bond15-toggle]").forEach((button)=>{
    button.addEventListener("click",(event)=>{
      ...
      state.selectedServantBond15[slotIndex]=!state.selectedServantBond15[slotIndex];
      renderAll();
    });
  });

  dom.servantSlots.querySelectorAll("[data-servant-bond-display-toggle]").forEach((button)=>{
    button.addEventListener("click",(event)=>{
      event.stopPropagation();
      const slotIndex=Number(event.currentTarget.dataset.servantBondDisplayToggle);
      state.selectedServantBondHidden[slotIndex]=!state.selectedServantBondHidden[slotIndex];
      renderAll();
    });
  });
}
...
function bindSlotEvents(container){
  container.querySelectorAll("[data-slot-type]").forEach((button)=>{
    button.addEventListener("click",(event)=>{
      ...
      if(type==="servant"){state.activeServantSlot=index;state.activeCESlot=null;}
      else{state.activeCESlot=index;state.activeServantSlot=null;}
      renderAll();
    });
  });
  ...
  container.querySelectorAll(".remove-entry").forEach((button)=>{
    button.addEventListener("click",(event)=>{
      ...
      if(type==="servant"){state.selectedServants[index]=null;state.selectedServantBond15[index]=false;state.selectedServantBondHidden[index]=false;}
      else{state.selectedCEs[index]=null;state.selectedCEOwned[index]=false;}
      renderAll();
    });
  });
}
...
function renderServantSidebar(){
  ...
  const totalServants=visibleServants.length,pageSize=Math.max(1,Number(state.servantSidebarPageSize)||SIDEBAR_PAGE_SIZE_OPTIONS[0]),totalPages=Math.max(1,Math.ceil(totalServants/pageSize)),currentPage=Math.min(Math.max(1,state.servantSidebarPage),totalPages);
  ...
  dom.servantResults.querySelectorAll("[data-add-servant]").forEach((button)=>{
    button.addEventListener("click",()=>{
      ...
      state.selectedServantBondHidden[targetIndex]=false;
...
function renderCESidebar(){
  ...
  dom.ceResults.querySelectorAll("[data-add-ce]").forEach((button)=>{
    button.addEventListener("click",()=>{
      ...
      state.selectedCEOwned[targetIndex]=isDefaultOwnCE(ce);
      renderAll();
    });
  });
}
...
function sidebarLoadingMarkup(label,progress){
  ...
  return `<div class="sidebar-loading-indicator" style="--loading-progress:${clampedProgress}"><div class="sidebar-loading-ring"><span class="sidebar-loading-ring-core">${clampedProgress}%</span></div><div class="small text-muted">${label}...</div></div>`;
}
...
function renderRecommendations(){
  ...
  if(!recommendations.length){
    dom.recommendationArea.classList.add("recommendation-grid-empty");
    dom.recommendationArea.innerHTML=`<div class="empty-state">Click Optimize CEs to rank bond-focused Craft Essences.</div>`;
    return;
  }
  ...
  dom.recommendationArea.querySelectorAll("[data-add-recommended-ce]").forEach((button)=>{
    button.addEventListener("click",()=>{
      const ceId=Number(button.dataset.addRecommendedCe),ce=state.ces.find((entry)=>entry.id===ceId);
      if(!ce)return;
      const targetIndex=state.activeCESlot??firstOpenSlot(state.selectedCEs);
      state.selectedCEs[targetIndex]=ce;
      state.selectedCEOwned[targetIndex]=isDefaultOwnCE(ce);
...
function highlightAffectedServants(ce){
  ...
  state.selectedServants.forEach((servant,servantSlotIndex)=>{
    if(!servant)return;
    if(doesCEAffectServant(ce,servant,-1,servantSlotIndex,true))dom.servantSlots.querySelector(`.selection-slot[data-slot-index="${servantSlotIndex}"]`)?.classList.add("highlighted-slot");
  });
}
