function getHypotheticalCEBonusForServant(ce, servant, servantSlotIndex) { if (!ce || !servant) return 0; if (!doesCEAffectServant(ce, servant, -1, servantSlotIndex, true)) return 0; return Number(ce.percent) || 0; }

function buildCERecommendations() {
  const selectedServants = state.selectedServants.map((servant, slotIndex) => ({ servant, slotIndex })).filter((entry) => entry.servant);
