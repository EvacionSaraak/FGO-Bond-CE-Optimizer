...
async function loadAtlasData(){
  ...
  try{
    ...
    try{
      ...
      servants=await fetchLocalJson(FALLBACK_SERVANT_JSON_URL,{resourceLabel:"Servants"});loadedFromFallback=true;setStatus("Atlas servants unavailable.\nLoaded local servant fallback.\nLoading Craft Essences...","warning");
    }
    ...
    state.servantSidebarLoading=false;state.ceSidebarLoading=false;state.servantSidebarLoadingProgress=0;state.ceSidebarLoadingProgress=0;setStatus(`Failed to load data from Atlas Academy and local fallbacks: ${error.message}`,"danger");
  }
}

async function refreshCraftEssencesInBackground(){
  try{const remoteCraftEssences=await fetchJsonWithRetry(CE_API_URL,{resourceLabel:"Craft Essence"});state.ces=normalizeCEs(remoteCraftEssences);state.ceSidebarPage=1;setStatus(`Craft Essences updated from Atlas Academy (${state.ces.length.toLocaleString()} entries).`,"success");renderCESidebar();}
  catch(_error){setStatus(`Using local Craft Essence snapshot (${state.ces.length.toLocaleString()} entries).\nAtlas update check failed.`,"warning");}
}
...
async function fetchJsonWithRetry(url,{resourceLabel,updateProgress}){
  ...
  while(attempt<=API_RETRY_MAX_ATTEMPTS){
    ...
    catch(_error){if(attempt>=API_RETRY_MAX_ATTEMPTS)throw new Error(`${resourceLabel} request failed after ${API_RETRY_MAX_ATTEMPTS} attempts.`);const retrySeconds=Math.max(1,Math.round(delayMs/1000));setStatus(`${resourceLabel} request failed (attempt ${attempt}).\nRetrying in ${retrySeconds}s...`,"warning");await wait(delayMs);attempt+=1;delayMs=Math.min(delayMs*2,API_RETRY_MAX_DELAY_MS);}
  }
...
function normalizeServants(servants){
  return (Array.isArray(servants)?servants:[])
    ...
    .map((servant)=>{
      ...
      const displayTraits=[...new Set(rawTraits.map((trait)=>humanizeTrait(trait?.nameEn||trait?.name||trait?.detail||trait?.displayName||trait?.shortName||"")).filter(Boolean))];
      const traitTokens=[...new Set(rawTraits.flatMap((trait)=>[trait?.id,trait?.trait,trait?.name,trait?.nameEn,trait?.detail,trait?.displayName,trait?.shortName,trait?.type]).filter((value)=>value!==null&&value!==undefined&&value!=="").flatMap(getTraitTokensFromValue))];
      const traitIds=[...new Set(rawTraits.map((trait)=>trait?.id??trait?.trait).filter((value)=>value!==null&&value!==undefined&&value!==""))];
      return {
        id:servant.id,
        name:servant.name,
        normalizedName:normalizeText(servant.name),
        rarity:Number(servant.rarity??servant.rarityRank??0),
        ...
        alignment:(()=>{if(Array.isArray(servant.alignment)&&servant.alignment.length)return servant.alignment.map((a)=>humanizeTrait(String(a||""))).filter(Boolean);if(rawTraits.length)return rawTraits.filter((t)=>String(t?.name||"").toLowerCase().startsWith("alignment")).map((t)=>humanizeTrait(String(t.name))).filter(Boolean);return [];})(),
...
function normalizeCEs(craftEssences){
  return (Array.isArray(craftEssences)?craftEssences:[])
    ...
    .map((ce)=>{
      ...
      const percentInfo=extractBondPercents(detail,ce.name),functionBasePercent=extractBondPercentFromFunctions(ce.skills),basePercent=percentInfo.basePercent||functionBasePercent,mlbMultiplier=getBondMLBMultiplier(ce.name),mlbPercent=isFlatBondPointCE(ce.name)?0:basePercent*mlbMultiplier;
      if(mlbPercent<=0)return null;
      const normalizedName=normalizeText(ce.name),hasTeatimeOwnPenalty=normalizedName==="chaldea teatime"&&mlbPercent>=15,ownBasePercent=hasTeatimeOwnPenalty?1:percentInfo.ownBasePercent||basePercent,ownMlbPercent=hasTeatimeOwnPenalty?5:ownBasePercent*mlbMultiplier,supportConditional=hasTeatimeOwnPenalty||percentInfo.isSupportConditional;
      return {
        id:ce.id,
        name:ce.name,
        normalizedName,
        rarity:Number(ce.rarity??ce.rarityRank??0),
        detail,
