...
async function loadAtlasData(){
  ...
  try{
    ...
    setStatus(
      loadedFromFallback
        ? `Loaded ${state.servants.length.toLocaleString()} servants (local fallback) and ${state.ces.length.toLocaleString()} local Craft Essences.\nChecking Atlas for CE updates...`
        : `Loaded ${state.servants.length.toLocaleString()} servants and ${state.ces.length.toLocaleString()} local Craft Essences.\nChecking Atlas for CE updates...`,
...
async function fetchLocalJson(url,{resourceLabel}){
  const response=await fetch(url,{cache:"no-store"});
  if(!response.ok)throw new Error(`${resourceLabel} fallback HTTP ${response.status}`);
  return await response.json();
}
...
function hasBondGainFunction(skills){
  return Array.isArray(skills)&&skills.some((skill)=>(skill.functions??[]).some((func)=>isBondGainFunction(func)));
}
